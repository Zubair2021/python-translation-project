...
def vet_nucleotide_sequence(sequence):
    ...
    rna_pattern_str =  r'^(?<![Tt])[AUCGaucg]*$'
    dna_pattern_str =  r'^(?<![Uu])[ATCGatcg]*$'
...
def vet_codon(codon):
    ...
    # only valid codons.
    # Read the docstring above for additional clues.
    codon_pattern_str = r'^[AUGC][AUGC][AUGC]$'
    ##########################################################################
...
def find_first_orf(sequence,
    ...
    #orf_pattern_str = "mystring1".replace(re, "newstring");
    start_codons = ''.join([str(elem) for elem in start_codons])
    stop_codons = '|'.join([str(elem) for elem in stop_codons])
    orf_pattern_str = r'('+start_codons+')(...)*('+stop_codons+')'
